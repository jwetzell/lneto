...
func main() {
	...
	for i := 0; i < 5; i++ {
		iface, err := NewSerialInterface("/dev/cu.usbserial-B00210HG", 115200)
		if err != nil {
	...
	if network == nil {
		fmt.Println("failed to create TCP interface after retries")
		os.Exit(1)
	}
...
func run(ctx context.Context, stack *xnet.StackAsync) error {
	...
	err = stack.Reset(xnet.StackConfig{
		StaticAddress4: [4]byte{192,168,0,2},
		Hostname: "lneto-mwe",
	...
	raddr := net.TCPAddrFromAddrPort(netip.AddrPortFrom(netip.AddrFrom4([4]byte{192, 168, 0, 1}), 80))
	...
	c, err := berkstack.Socket(ctx, "tcp", syscall.AF_INET, sockstream, nil, raddr)
	if err != nil {
